//
//  ZMBlockCycleReferenceV2Rule.swift
//  SwiftLint
//
...
struct ZMBlockCycleReferenceV2Rule: SwiftSyntaxRule {
    ...
    static let description = RuleDescription(
        identifier: "ZM_Block_Cycle_Reference_V2_Rule",
        name: "ZM Block Cycle Reference V2 Rule",
        description: "在Block中可能有循环引用问题，请仔细甄别V2",
        kind: .lint,
...
extension ZMBlockCycleReferenceV2Rule {
...
extension ZMBlockCycleReferenceV2Rule.Visitor {
    ...
    func findOriginalFunctionCallIdentifier(calledExpression: ExprSyntax?) -> String? {
        ...
        while(true) {
            ...
            } else if  let specializeExpr = tmpcalledExpression?.as(GenericSpecializationExprSyntax.self) {
                tmpcalledExpression = specializeExpr.expression
            } else if let identifier = tmpcalledExpression?.as(DeclReferenceExprSyntax.self)  {
    ...
    func dealWithAssignmentExpr(node: ClosureExprSyntax, exprList : ExprListSyntax) {
        ...
        while(true) {
            if let identifier = loopTarget?.as(DeclReferenceExprSyntax.self) {
                targetIdentifier = identifier.baseName.text
                break
            }
        ...
        if targetIdentifier == "self" {
            if currentContext?.undeclareIdentifiers.contains("self") ?? false {
                let reason = "self与闭包之间可能存在循环引用，请仔细甄别"
            ...
            if currentContext?.undeclareIdentifiers.contains(targetIdentifier) ?? false {
                let reason = "\(targetIdentifier)与闭包之间可能存在循环引用，请仔细甄别"
                let violation = ReasonedRuleViolation(position: node.positionAfterSkippingLeadingTrivia, reason: reason)
                violations.append(violation)
    ...
    func dealWithFunctionCallExpr(node: ClosureExprSyntax) {
        ...
        /// 闭包作为参数传入方法
        if let tupleExprElementNode = node.parent?.as(LabeledExprSyntax.self),
           let tupleExprElementListNode = tupleExprElementNode.parent?.as(LabeledExprListSyntax.self),
        ...
        /// 尾随闭包
        if let functionCallExpr = node.parent?.as(FunctionCallExprSyntax.self),
           let trailingClosure =  functionCallExpr.trailingClosure,
        ...
        let uikitArray: [String] = ["dismiss","performBatchUpdates"]
        ...
        if let targetFunctionCallExpr {
            ...
            if let calledExpression = targetFunctionCallExpr.calledExpression.as(MemberAccessExprSyntax.self) {
                ...
                if ignoreFunctionArray.contains(calledExpression.declName.baseName.text) {
                    return
                }
            ...
            ///
            if let specializeExpr = targetFunctionCallExpr.calledExpression.as(GenericSpecializationExprSyntax.self),
               let identifierExpr = specializeExpr.expression.as(DeclReferenceExprSyntax.self),
               ignoreSpecializeArray.contains(identifierExpr.baseName.text) {
                return
            }
        }
        ...
        if let specializeExpr = targetExpr?.as(GenericSpecializationExprSyntax.self) {
            targetExpr = specializeExpr.expression
        }
        
        if let targetFunctionCallExpr {
            if let identifier = targetFunctionCallExpr.calledExpression.as(DeclReferenceExprSyntax.self)  {
